...
function getPaidVCTime(questquery,response,callback){
  ...
  var LeftWH;
  var VCWH;
  util.log('debug','getPaidVCTime get questquery:' + JSON.stringify(questquery));
  async.waterfall([
            ...
            login.getWHSetting(questquery,response,
              ...
              if( util.jsonexist(cb,'/errno') && util.jsonget(cb,'/errno') == 400){  //异常情况下
                ...
                if( util.jsonexist(cb,'/errno') && util.jsonget(cb,'/errno') == 200){   //非异常情况下返回
                  ...
                  //计算一天的小时数
                  avgHour = util.jsonget(cb,'/queryresult0/avgHour');
...
*/
        ...
        util.log('debug',selectSQL1);
          ...
          {
            //global.queryDBStatus = 'err';
            util.log('error',"err  = "+ JSON.stringify(sqlerr));
            results = sqlerr;
...
//带有callback  根据传入的日期和时间，检查区间重复的考勤加班休假申请

function getExistWHList(questquery,response,callback){
  var i = 0;
  var results ={
    errno:'',
    errmsg:''
  ...
  async.series([
      ...
      pool.getConnection(function(sqlerr, Connection) {
        ...
        selectSQL1 += ' where EmployeeID ='  + Connection.escape(util.jsonget(questquery,'/userid'))
        selectSQL1 += ' and ObjYMD between ' + Connection.escape(util.jsonget(questquery,'/startdt')) + ' and ' + Connection.escape(util.jsonget(questquery,'/enddt'))
        selectSQL1 += ' and ( ' + Connection.escape(util.jsonget(questquery,'/FromDt')) + ' <= ToDt and ' + Connection.escape(util.jsonget(questquery,'/ToDt'))  + ' > FromDt)';
        selectSQL1 += ' group by detail.EmployeeID,ObjYMD ';
        ...
        util.log('debug',selectSQL1);
            ...
            query
              ...
              .on('end', function(rows) {
                  ...
                  if(i>0){
                    util.jsonadd(results,'/errno','200');
                    util.jsonadd(results,'/errmsg','getExistWHList complete');
                    util.jsonadd(results,'/rowcount',i);
                    util.jsonadd(results,'/module','getExistWHList');
                  }
                  else
                  {
                    util.jsonadd(results,'/errno','300');
                    util.jsonadd(results,'/errmsg','getExistWHList Empty');
                    util.jsonadd(results,'/rowcount',0);
                    util.jsonadd(results,'/module','getExistWHList');
                  }
          ...
          if(sqlerr == null||sqlerr == '' ){
            ...
            util.log('info','getExistWHList returns');
            util.log('info',JSON.stringify(results[0]));
            callback(results[0]);
          }
          else
          {
            ...
            results = sqlerr;
            util.jsonadd(results,'/errno','400');
            util.jsonadd(results,'/errmsg','getExistWHList error');
              // util.jsonadd(results,'/rowcount',i);
            util.jsonadd(results,'/module','getExistWHList');
            util.log('info','getExistWHList returns');
            util.log('info',JSON.stringify(results));
...
exports.getExistWHList=getExistWHList;
